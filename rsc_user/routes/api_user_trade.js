...
module.exports = function() {
    ...
    api.post('/signup', function(req, res, next) {
        if(!config_common.checkCommonString(req.body.full_name) ||
            ...
            req.body.type != config_common.company_category.TRADE) {
    ...
    api.post('/signup_by_invitation',function(req, res, next) {
        ...
        Invitation.findById(req.body.id, function(err, invitation) {
            ...
            VerifyCode.findOne({phone: req.body.phone},function(err, v_code) {
                ...
                if(!v_code){
                    return next('verifyCode_not_found');
                }
                if(v_code.companyType){
                    return next('phone_is_used');
                }
                if(v_code.code != req.body.verify_code){
                    return next('invalid_verify_code');
                }
                var userData;
                if(invitation.type == config_common.company_category.TRADE){
                    userData = User;
                }else{
                    userData = UserTraffic;
                }
                ...
                user.save(function(err) {
                    ...
                    v_code.companyType = invitation.type;
                    v_code.save(function(){});
    ...
    api.post('/login',function(req, res, next) {
        ...
        VerifyCode.findOne({phone: req.body.phone},function(err, v_code) {
            ...
            var userData;
            if(v_code.companyType == config_common.company_category.TRADE){
                userData = User;
            }else{
                userData = UserTraffic;
            }
            userData.findOne({phone:req.body.phone})
